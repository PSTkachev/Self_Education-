...
import requests
lines = 0
with open ('dataset_3378_2.txt', 'r') as link: # считываем ссылку из скаченного файла
    link = link.read().strip()
file = requests.get(link) # GET запрос по полученой ранее ссылке
for i in file.text.splitlines(): # считаем строки в файле
    lines += 1
print(lines)