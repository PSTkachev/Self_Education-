...
Нужно зашифровать строку abacabadaba
и расшифровать строку #*%*d*% с помощью этого шифра.

Получаем следующие строки, которые и передаём на вывод программы:
*d*%*d*#*d*
dacabac"""

alphabet, key, msg1, msg2 = list(input()), list(input()), list(input()), list(input())
for i in msg1:
    print(key[alphabet.index(i)], end = '') # Для каждой буквы списка msg1, выводим то значение key,которое соответствует alphabet
                                            # Ещё раз для каждого символа в мсдж 1 мы находим порядковый номер этого символа в алфавите, а потом выводим значение ключа которое равно этому порядковому номеру...Profit
print()
for j in msg2:
    print(alphabet[key.index(j)], end = '') # Для каждой буквы списка str2, выводим то значение alphabet,которое соответствует key
print()
